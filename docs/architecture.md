...
- 초대형 워크북 압축(SpreadsheetLLM류)은 백로그 — 공식 코드 미공개로
  커뮤니티 재구현체 검토 필요.
- v2(서식·의도 채널·수식 지도)는 system_design 4.1절 — 감사조서의 "수행된 절차의
  흔적"이 주로 서식(색상 마킹)·메모에 기록되므로, 프롬프트가 지시하는 관찰을
  도구가 감각기관으로 뒷받침한다. 참조 구현(`docs/reference/xlsx_agent_tools.py`) 이식.
